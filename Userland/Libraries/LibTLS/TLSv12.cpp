...
ErrorOr<NonnullOwnPtr<WolfTLS>> WolfTLS::connect(const AK::ByteString& host, u16 port)
{
    ...
    wolfSSL_SSLSetIOSend(ssl, [](WOLFSSL*, char* buf, int sz, void* ctx) -> int {
        ...
        auto result = self.underlying->write_some({ buf, static_cast<size_t>(sz) });
    ...
    wolfSSL_SSLSetIORecv(ssl, [](WOLFSSL*, char* buf, int sz, void* ctx) -> int {
        auto& self = *static_cast<WolfTLS*>(ctx);
        auto result = self.underlying->read_some({ buf, static_cast<size_t>(sz) });
        if (result.is_error() && result.error().is_errno())
            return errno_to_wolfssl_error(result.error().code());
        if (result.is_error())
            return -1;
...
{
    auto result = wolfSSL_read(ssl, bytes.data(), bytes.size());
    if (result < 0)
        return Error::from_string_view(error_text(ssl, result));
    return Bytes { bytes.data(), static_cast<size_t>(result) };
}

ErrorOr<size_t> WolfTLS::write_some(ReadonlyBytes bytes)
{
    auto result = wolfSSL_write(ssl, bytes.data(), bytes.size());
    if (result < 0)
        return Error::from_string_view(error_text(ssl, result));
...
{
    return wolfSSL_get_error(ssl, 0) == SSL_ERROR_WANT_READ;
}

bool WolfTLS::is_open() const
{
    return wolfSSL_get_error(ssl, 0) != SSL_ERROR_ZERO_RETURN;
}
...
ErrorOr<void> WolfTLS::set_close_on_exec(bool)
{
    return {};
}
